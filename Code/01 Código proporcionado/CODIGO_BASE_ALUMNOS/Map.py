...
class Map:
    ...
    def compute_detection_map(self) -> np.array:
        ...
        lat_coords = np.linspace(self.boundaries.min_lat, self.boundaries.max_lat, self.height)
        lon_coords = np.linspace(self.boundaries.min_lon, self.boundaries.max_lon, self.width)
        ...
        if max_prob != min_prob:
            detection_map = ((detection_map - min_prob) / (max_prob - min_prob)) * (1 - EPSILON) + EPSILON
        else:
